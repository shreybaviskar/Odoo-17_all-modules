# Company: Areterix Techonlogies
# Developer: mohammad
# Changes made in this file for tcs value,  notes and sections in item list.
# don't use user notify method for popup or alert message.Only use return.
# search using developer name:  mohammad
    ...
    def create_einvoicing(self):
        ...
        if self.partner_id.country_id.code != 'IN':
            ...
            BuyerDtls = {
                ...
                #Mohammad Code
                "LglNm": self.partner_id.parent_id.name or self.partner_id.name,
        ...
        # old shipping details by the developer which were same as billing details
        # so updating the shipping details.
        ...
        if self.partner_shipping_id.street:
            addr1 = self.partner_shipping_id.street
        if self.partner_shipping_id.street and self.partner_shipping_id.street2:
            addr1 = self.partner_shipping_id.street + ',' + self.partner_shipping_id.street2

        ShipDtls = {
            "LglNm": self.partner_shipping_id.name,
            "Addr1": addr1,
            "Loc": self.partner_shipping_id.city,
            "Pin": int(self.partner_shipping_id.zip),
            "Stcd": self.partner_shipping_id.state_id.l10n_in_tin
        }
        if self.transaction_type == '1':
            data['SellerDtls'] = SellerDtls
            data['BuyerDtls'] = BuyerDtls
            # data['DispDtls'] = DispDtls #not required in regular
            # data['ShipDtls'] = ShipDtls #not required in regular
        ...
        for idx, inv_line in enumerate(self.invoice_line_ids):
            if not inv_line:
                if inv_line.price_subtotal > 5:
                    tcs_amount += round((rec.amount_untaxed * rec.tds_per) / 100, 2)
                else:
        ...
        for idx, inv_line in enumerate(self.invoice_line_ids):
            if inv_line:
                # mohammad
                # if notes and section exists in invoice  E-invoice api used to
                # throw error that HSN not valid. So Placed this conditionto skip
                # the section and notes from sending it to einvoice api
                if inv_line.product_id.id != False:
                    print("Values :", inv_line.product_id.id)
                    print("Values1 :", inv_line.name)
                    ...
                    if inv_line.tax_ids:
                        for tax in inv_line.tax_ids:
                            ...
                            # STart Mohammad Code Remove else condition to skip other taxes in tax rate
                            # else:
                            #     tax_rate = tax.amount
                            # STart Mohammad Code Remove else condition to skip other taxes in tax rate
                            if tax.amount_type != 'group':
                                if tax.tax_group_id.name == "IGST" and tax.price_include == False:
                                    ...
                                    assmt = round(inv_line.price_subtotal - (
                                            (inv_line.price_unit * inv_line.quantity) - inv_line.price_subtotal), 2)
                                    # Start Mohammad Code Set tax rate igst value
                                    tax_rate = tax.amount
                                    # End Mohammad Code
        ...
        # Start Code Mohammad Add TCS Value In E Invoice

        data['ItemList'] = item_list
        tcs_lines = self.env['account.move.line'].search([
            ('move_id', '=', self.id),
            ('name', '=', 'TCS 1%')
        ])
        # print('tcs_lines',tcs_lines)
        local_tcs_value1 = 0
        if tcs_lines:
            local_tcs_value1 = sum(tcs_lines.mapped('credit'))

        # If not found, set to 0
        local_tcs_value = round(local_tcs_value1, 2) if local_tcs_value1 else 0.0

        # End Code Mohammad Add TCS Value In E Invoice

        # tcs_lines = self.env['account.move.line'].search([
        #     ('move_id', '=', self.id),
        #     ('name', '=', 'TCS 1%')
        # ])
        # tcs_amount = sum(tcs_lines.mapped('credit'))
        #
        # # If not found, set to 0
        # local_tcs_value = round(tcs_amount, 2) if tcs_amount else 0.0
        # mohammad
        # add condition for tcs value if not set then set 0
        # tcs_amount += round((self.amount_untaxed * rec.tds_per) / 100, 2)
        # if self.tcs_value == False:
        #     local_tcs_value = 0
        # else:
        #     local_tcs_value = self.tcs_value

        values = {
            "AssVal": total,
            "RndOffAmt": round(total_round, 2),
            "Othchrg": round(local_tcs_value, 2),
        ...
        if res_dict.get('Status') == '1':
            ...
            if self.eway_bill_no:
                return {
                    ...
                    'params': {'type': 'info', 'message': _("IRN Number Generated Successfully  !", ),
                               'sticky': False, }
                }
                # mohammed: Below code used to throw error
